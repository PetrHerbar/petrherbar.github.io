...
const products = {
  ...
  'vlasovy-olej': {
    name: 'Vlasový olej',
    description: 'Výživný olej pro lesklé a zdravé vlasy s přírodními oleji a vitamíny. Poskytuje intenzivní výživu a ochranu vlasů.',
    price: '419 Kč',
    image: 'assets/Products/VlasovyOlej.png',
    features: [
      'Přírodní oleje a vitamíny',
      'Intenzivní výživa vlasů',
      'Chrání před poškozením',
      'Lesklé a zdravé vlasy',
...
class BubbleSystem {
  ...
  createBubbles() {
    ...
    let bubbleCount;
    if (isMobile) {
      bubbleCount = isLowEnd ? 0 : 7;
    } else {
      bubbleCount = isLowEnd ? 7 : 40;
    }
...
function showPurchaseMessage(productId, action = 'Přidat do košíku') {
  ...
  message.style.cssText = `
    ...
    top: 20px;
    right: 20px;
    background: linear-gradient(135deg, var(--green), #0c3821);
    color: white;
    padding: 1rem 1.5rem;
    border-radius: 10px;
